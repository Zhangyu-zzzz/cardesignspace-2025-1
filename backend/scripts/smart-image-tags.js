...
["外型", "正前", "前脸", "运动", "时尚"]`;
    ...
    try {
      ...
      console.log(`📝 分析响应: ${content.substring(0, 150)}...`);
  ...
  async run() {
    try {
      console.log('🚀 开始智能图片标签生成...');
      console.log('💡 此脚本将使用DeepSeek API分析图片内容生成准确标签');
      ...
      // 手动筛选需要处理的图片
      const imagesToProcess = allImages.filter(image => {
      ...
      console.log(`📊 找到 ${imagesToProcess.length} 张需要处理的图片`);
      ...
      // 只处理前3张图片进行测试
      const testImages = imagesToProcess.slice(0, 3);
      console.log(`🔧 测试处理前 ${testImages.length} 张图片`);
...
if (require.main === module) {
  const tagger = new SmartImageTagger();
  tagger.run().catch(console.error);
}

module.exports = SmartImageTagger;
