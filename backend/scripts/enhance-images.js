...
class ImageEnhancer {
  ...
  async getImageTags(imageUrl, modelName, brandName) {
    const prompt = `请为"${brandName} ${modelName}"车型的图片生成标签。

请从以下角度分析并生成标签：
1. 图片类型：外型、内饰、零部件、其他
2. 拍摄角度：正前、正侧、正后、前45、后45、俯侧、顶视
3. 具体部位：前脸、侧面、尾部、内饰、仪表盘、座椅、方向盘等
4. 风格特征：运动、豪华、商务、家用、时尚、科技等

请以JSON数组格式返回标签，例如：
["外型", "正前", "前脸", "运动", "时尚"]

注意：请基于车型信息进行合理推测，生成3-5个相关标签。`;
  ...
  async updateImageTags(imageId, tags) {
    try {
      ...
      console.log(`✅ 图片 ${imageId} 标签更新成功: ${tags.join(', ')}`);
      return true;
    } catch (error) {
  ...
  async processImage(image) {
    try {
      ...
      // 获取图片标签
      const tags = await this.getImageTags(
        image.url, 
        image.Model?.name || '未知车型', 
        image.Model?.Brand?.name || '未知品牌'
      );
