...
/**
 * 最终混合标签生成系统
 * 结合训练好的模型和DeepSeek API，提高标签生成准确度
 */
...
class FinalHybridSystem {
   ...
   * 调用训练好的模型进行预测
   */
  async predictWithTrainedModel(imageUrl, imageId) {
    return new Promise((resolve, reject) => {
      console.log(`🤖 调用训练好的模型分析图片 ${imageId}...`);
      ...
      // 调用训练好的模型脚本
      const pythonScript = path.join(AI_MODEL_PATH, 'predict_with_trained_model.py');
        ...
        if (code === 0) {
          try {
            ...
            if (jsonStart !== -1 && jsonEnd > jsonStart) {
              ...
              console.log(`✅ 训练模型预测完成: ${result.top_labels?.join(', ') || '无标签'}`);
              if (result.confidences) {
                const maxConf = Math.max(...Object.values(result.confidences));
...
["外型", "正前", "前脸", "运动", "时尚"]`;
   ...
   * 智能融合两种预测结果
   */
  fusePredictions(trainedModelResult, deepseekResult) {
    ...
    // 处理训练模型的结果
    if (trainedModelResult && trainedModelResult.success && trainedModelResult.top_labels) {
      trainedModelResult.top_labels.forEach(tag => {
        const confidence = trainedModelResult.confidences?.[tag] || 0.5;
        fusedTags.add(tag);
        tagConfidence.set(tag, confidence * 0.8); // 训练模型权重0.8（更高）
      });
    }
    ...
    if (deepseekResult && deepseekResult.length > 0) {
      deepseekResult.forEach(tag => {
        ...
        tagConfidence.set(tag, existingConfidence + 0.2); // DeepSeek权重0.2
      });
    }
  ...
  async processImage(image) {
    try {
      ...
      const [trainedModelResult, deepseekResult] = await Promise.allSettled([
        this.predictWithTrainedModel(image.url, image.id),
        this.predictWithDeepSeek(image.url, modelName, brandName, image.filename)
      ]);
      
      // 处理预测结果
      const trainedTags = trainedModelResult.status === 'fulfilled' ? trainedModelResult.value : null;
      const deepseekTags = deepseekResult.status === 'fulfilled' ? deepseekResult.value : [];
      
      console.log('\n📊 预测结果分析:');
      console.log('─'.repeat(50));
      
      if (trainedModelResult.status === 'fulfilled') {
        console.log(`✅ 训练模型: ${trainedTags?.top_labels?.join(', ') || '无标签'}`);
        if (trainedTags?.confidences) {
          const maxConf = Math.max(...Object.values(trainedTags.confidences));
          console.log(`   最高置信度: ${maxConf.toFixed(3)}`);
        }
      } else {
        console.log(`❌ 训练模型: ${trainedModelResult.reason.message}`);
      }
      ...
      // 融合预测结果
      const fusedTags = this.fusePredictions(trainedTags, deepseekTags);
  ...
  async run() {
    try {
      console.log('🚀 开始最终混合标签生成系统...');
      console.log('🤖 结合训练好的模型和DeepSeek API，提高标签准确度');
...
if (require.main === module) {
  const system = new FinalHybridSystem();
  system.run().catch(console.error);
}

module.exports = FinalHybridSystem;
