...
# 1. 检查nginx.production.conf是否已上传
...
# 3. 检查配置内容（看是否包含安全规则）
grep "阻止敏感文件" /etc/nginx/sites-available/cardesignspace
# 如果没有输出，说明配置还是旧的
...
问题1: nginx -t 报错
解决: 检查配置文件语法，特别注意location块的顺序

问题2: 配置正确但不生效
解决: 需要 reload 或 restart nginx
      sudo systemctl reload nginx
...
echo ""
echo "是否现在就SSH到服务器进行手动检查? (需要输入密码)"
echo ""
read -p "按回车继续SSH到服务器，或Ctrl+C取消: "
